...
NoLockRing::NoLockRing(uint32_t size, uint32_t flag) : 
        m_size(0), m_flag(flag), m_buf(nullptr)
{
...
{
        uintptr_t *pdst = reinterpret_cast<uintptr_t *>(dst);
        uintptr_t**psrc = reinterpret_cast<uintptr_t**>(src);
        pdst[dstIndex] = reinterpret_cast<uintptr_t>(psrc[srcIndex]); 
}
